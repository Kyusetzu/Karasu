...
export function ScoreSelect({
  ...
  if (options === null) {
    ...
    return (
      <input
        ...
        className={cn(
          "h-8 rounded-inner border border-surface-800 bg-surface-900 px-2 text-xs tabular-nums text-gold transition-surface focus:border-accent-500 focus:outline-none",
  ...
  return (
    <select
      ...
      onClick={(e) => e.stopPropagation()}
      className={cn(
        "h-8 rounded-inner border border-surface-800 bg-surface-900 px-2 text-xs tabular-nums text-gold transition-surface focus:border-accent-500 focus:outline-none disabled:opacity-50",
        className,
      )}
      aria-label={t("common.score")}
      title={t("common.score")}
